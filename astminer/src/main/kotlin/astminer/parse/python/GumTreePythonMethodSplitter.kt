...
class GumTreePythonMethodSplitter : TreeMethodSplitter<GumTreePythonNode> {
    ...
    private fun collectMethodInfo(methodNode: GumTreePythonNode): MethodInfo<GumTreePythonNode> {
        val methodReturnType = getElementType(methodNode) // no methods types for current parser
    ...
    private fun getElementName(node: GumTreePythonNode) = node // name node is node itself for many python nodes

    private fun getElementType(node: GumTreePythonNode): GumTreePythonNode? {
        if (node.getTypeLabel() == TypeLabels.arg) {
            return node.getChildren().firstOrNull { it.getTypeLabel() == TypeLabels.nameLoad } as GumTreePythonNode?
        }
